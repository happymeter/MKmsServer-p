...
import org.meter.kms.tools.filetimes
import org.meter.kms.kmsPidGenerator
from org.meter.kms.tools.formatText import justify, shell_message, byterize
...
applicationId TEXT, skuId TEXT, licenseStatus TEXT, lastRequestTime INTEGER, kmsEpid TEXT, requestCount INTEGER)")
        ...
        requestDatetime = org.meter.kms.tools.filetimes.filetime_to_dt(kmsRequest['requestTime'])
...
clientMachineId=:clientMachineId;", infoDict)
    ...
    def createKmsResponse(self, kmsRequest):
        ...
        if not self.config["epid"]:
            response["kmsEpid"] = org.meter.kms.kmsPidGenerator.epidGenerator(kmsRequest['applicationId'].get(),
                                                                              kmsRequest['versionMajor'],
...
import org.meter.kms.request.kmsRequestV4, org.meter.kms.request.kmsRequestV5, org.meter.kms.request.kmsRequestV6, \
    org.meter.kms.request.kmsRequestUnknown
...
def generateKmsResponseData(data, config):
    ...
    if version == 4:
        ...
        messagehandler = org.meter.kms.request.kmsRequestV4.kmsRequestV4(data, config)
        messagehandler.executeRequestLogic()
    elif version == 5:
        logging.info("Received V%d request on %s." % (version, currentDate))
        messagehandler = org.meter.kms.request.kmsRequestV5.kmsRequestV5(data, config)
        messagehandler.executeRequestLogic()
    elif version == 6:
        logging.info("Received V%d request on %s." % (version, currentDate))
        messagehandler = org.meter.kms.request.kmsRequestV6.kmsRequestV6(data, config)
        messagehandler.executeRequestLogic()
    else:
        logging.info("Unhandled KMS version V%d." % version)
        messagehandler = org.meter.kms.request.kmsRequestUnknown.kmsRequestUnknown(data, config)
